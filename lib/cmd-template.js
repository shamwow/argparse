...
module.exports = function (template, fn){
	...
	self._formatExp = new RegExp('^ *(\\w+)(?: +(?:< *(\\w+) *(?:, *([1-9]+))? *>))*( +(\\[ *(\\w+) *(?:, *([1-9]+))? *\\]))?(\\.\\.\\.)? *$');
	...
	self._oArgExp = new RegExp('\\[ *(\\w+) *(?:, *([1-9]+))? *\\](?:(\\.\\.\\.)$)? *$', 'g');
	...
	self.name = self._nameExp.matchAll(template)[0];
	...
	/*
		...
		Components of argument template accessible by sing capture groups
	*/
	self._requiredProps.forEach(function (elem){

		var name = elem[0],
				number = parseInt(elem[1]) || 0;

		self.argTemplates.push({
			name: name,
			required: true,
			tuple: number,
			infinite: elem[1] == '...' || elem[2] == '...'
		});
	});

	if(self._optionalProp.length){

		var elem = self._optionalProp[0],
				name = elem[0],
				number = parseInt(elem[1]) || 0;
